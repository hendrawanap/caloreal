    ...
    <form wire:submit.prevent="store" class="flex flex-col w-full" action="{{ route('bmr.store') }}" method="POST" class="flex flex-col">
        ...
        <div class="flex flex-shrink-0 text-white">
            <label for="male" class="mr-4">
                <div
                    class="flex flex-col py-2 px-2 bg-gray-50 rounded-xl border border-primary items-center text-primary justify-center sm:px-10">
                ...
                <input
                    ...
                    type="radio" id="male" name="sex" value="male" @if (old('sex')) checked @endif>
            </label>
            ...
            <label for="female">
                ...
                <input
                    wire:model="sex"
                    class="w-full py-2 px-4 rounded-xl border border-primary bg-gray-50 placeholder-primary focus:placeholder-opacity-25"
                    type="radio" id="female" name="sex" value="female" @if (old('sex')) checked @endif>
            </label>
        </div>
        ...
        <input
            wire:model="age"
            class="w-full py-2 px-4 rounded-xl border border-primary bg-gray-50 placeholder-primary focus:placeholder-opacity-25"
            type="text" name="age" id="age" value="{{ old('age') }}" placeholder="(dalam tahun)">
        ...
        <input
            wire:model="height"
            class="w-full py-2 px-4 rounded-xl border border-primary bg-gray-50 placeholder-primary focus:placeholder-opacity-25"
            type="text" name="height" id="height" value="{{ old('height') }}" placeholder="(dalam cm)">
        ...
        <input
            wire:model="weight"
            class="w-full py-2 px-4 rounded-xl border border-primary bg-gray-50 placeholder-primary focus:placeholder-opacity-25"
            type="text" name="weight" id="weight" value="{{ old('weight') }}" placeholder="(dalam kg)">
        @error('weight') <span class="text-sm text-red-500">{{ $message }}</span> @enderror

        <input type="submit" name="submit" value="Hitung IMT"
